...
func (k msgServer) CouncilResponseReveal(goCtx context.Context, msg *types.MsgCouncilResponseReveal) (*types.MsgCouncilResponseRevealResponse, error) {
	...
	if council.Status != types.CouncelingStatus_commited {
		return nil, errorsmod.Wrapf(sdkerrors.ErrUnauthorized, "invalid status, ave '%s', want '%s'", council.Status.String(), types.CouncelingStatus_commited.String())
	}
	...
	if slices.Contains(allreadyVoted, msg.Creator) {
		return nil, errorsmod.Wrap(sdkerrors.ErrUnauthorized, "Already voted")
	}
	...
	if err != nil {
		return nil, errorsmod.Wrap(sdkerrors.ErrInsufficientFunds, "Voter does not have enough coins")
	}
