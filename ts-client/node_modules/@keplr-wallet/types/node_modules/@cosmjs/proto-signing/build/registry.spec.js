...
describe("registry demo", () => {
    it("works with a default msg", () => {
        ...
        utils_1.assert(Coin);
        utils_1.assert(MsgSend);
        utils_1.assert(registry_1.isTsProtoGeneratedType(Coin));
        utils_1.assert(registry_1.isTsProtoGeneratedType(MsgSend));
        ...
        utils_1.assert(msg.typeUrl);
        utils_1.assert(msg.value);
    ...
    it("works with a custom msg", () => {
        ...
        utils_1.assert(MsgCreatePost);
        utils_1.assert(registry_1.isPbjsGeneratedType(MsgCreatePost));
        const msgDemo = MsgCreatePost.create({
            creator: "Me",
            title: "Something with stars",
            body: "la la la",
            attachment: encoding_1.fromHex("AABBAABB66FE"),
        });
        ...
        expect(Uint8Array.from(msgDemoDecoded.attachment)).toEqual(encoding_1.fromHex("AABBAABB66FE"));
    });
});
