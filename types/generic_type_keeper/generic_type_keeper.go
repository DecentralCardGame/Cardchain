...
func NewGTK[T proto.Message](key storetypes.StoreKey, internalKey storetypes.StoreKey, cdc codec.BinaryCodec, getEmpty func() T) GenericTypeKeeper[T] {
	gtk := GenericTypeKeeper[T]{
		Key:         key,
...
func (gtk GenericTypeKeeper[T]) GetIterator(ctx sdk.Context) storetypes.Iterator {
	store := ctx.KVStore(gtk.Key)
	return storetypes.KVStorePrefixIterator(store, nil)
}
